...
def test_migration_manager_history_filter_search_and_reopen(tmp_path):
 ...
 with sqlite3.connect(p.database_path) as db:assert db.execute("SELECT value FROM schema_metadata WHERE key='schema_version'").fetchone()==("11",);assert db.execute("SELECT count(*) FROM rating_strategies").fetchone()==(1,)
