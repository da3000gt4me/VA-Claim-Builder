...
def test_migration_crud_relationships_revisions_duplicate_reopen(tmp_path):
 ...
 with sqlite3.connect(p.database_path) as c:assert c.execute("SELECT value FROM schema_metadata WHERE key='schema_version'").fetchone()==("11",);assert c.execute("SELECT count(*) FROM nexus_letter_versions").fetchone()[0]==2
