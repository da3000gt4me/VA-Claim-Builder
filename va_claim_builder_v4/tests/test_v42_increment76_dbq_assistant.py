...
def test_migration_crud_relationships_revisions_duplicate_and_reopen(tmp_path):
 ...
 with sqlite3.connect(p.database_path) as c:assert c.execute("SELECT value FROM schema_metadata WHERE key='schema_version'").fetchone()==("7",);assert {"dbq_records","dbq_revisions","dbq_generations"}<={r[0] for r in c.execute("SELECT name FROM sqlite_master WHERE type='table'")}
