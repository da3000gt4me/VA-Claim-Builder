...
class DocumentImportWorker(QObject):
    ...
    def __init__(self, project: ProjectInfo, filenames: list[str]) -> None:
        super().__init__()
        ...
        try:
            ...
            for index, filename in enumerate(self.filenames, start=1):
                if self._cancel_event.is_set():
                    ...
                    return
                path = Path(filename)
                percent_before = int(((index - 1) / total) * 100)
            ...
            self.jobs.update(self.job.job_id, status="completed", progress=100, message="Import complete")
            self.completed.emit(imported_count, duplicate_count)
